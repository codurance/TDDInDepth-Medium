package com.codurance.solid.lcp.untestable;

public class InvoiceService {
    private final InvoiceFactory invoiceFactory;

    public InvoiceService(InvoiceFactory invoiceFactory) {
        this.invoiceFactory = invoiceFactory;
    }

    public void sendInvoice(Customer customer) {
        Invoice invoice = invoiceFactory.createInvoice(customer.preferredInvoice);

        if (invoice instanceof PaperInvoice paperInvoice) {
            paperInvoice.sendPaperInvoice(customer.address);
        } else if (invoice instanceof EmailInvoice emailInvoice) {
            emailInvoice.sendEmailInvoice(customer.emailAddress);
        }
    }
}
